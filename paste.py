...
class PasteBin:
    ...
    def api_call(self, method, params):
        response = urllib.request.urlopen('https://pastebin.com/api/' + method,
                                          urllib.parse.urlencode(params).encode('utf-8'), timeout=self.timeout)
    ...
    def delete_paste(self, paste_key):
        params = {'api_dev_key': self.api_dev_key, 'api_user_key': self.api_user_key, 'api_paste_key': paste_key,
                  'api_option': 'delete'}
        return self.api_call('api_post.php', params)
...
# API Settings
api_dev_key = 'f38f2f7903844ed894eba2cb24ff5bb2'
api_user_key = '7fbde71dba5d2693161386cb0ca77e00'

# Define API
if api_user_key:
    api = PasteBin(api_dev_key, api_user_key)
else:
    api = PasteBin(api_dev_key)
    username = "WindCry1"
    password = "yh13706207565"
...
data = open(file).read()
result = api.paste(data, guest=False, name=file.split("/")[-1], format='Cpp', private='1', expire='1M')
